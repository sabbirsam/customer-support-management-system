...
function Header()
    {
        ...
        $this->Cell(276,5,'Supplier Information',0,0,'C');
...
// Page footer
    ...
    function viewTable($db){
        ...
        $stmt = $db->query("SELECT *,concat(lastname,', ',firstname,' ',middlename) as name FROM supplier order by concat(lastname,', ',firstname,' ',middlename) asc");
        while($data = $stmt->fetch(PDO::FETCH_OBJ)){

            $srno = ("WDSUPN00". $data->id);

            $this->Cell(25,10,$srno,1,0,'L');
            $this->Cell(60,10,$data->firstname,1,0,'L');
            $this->Cell(35,10,$data->contact,1,0,'L');
            $this->Cell(40,10,$data->address,1,0,'L');
            $this->Cell(90,10,$data->email,1,0,'L');
            $this->Ln();
...
$pdf->Output();
?>
<!-- $result = mysqli_query($conn, "SELECT id, product_id,firstname,lastname,middlename, contact, address, email, date_created FROM supplier") or die("database error:". mysqli_error($conn)); -->