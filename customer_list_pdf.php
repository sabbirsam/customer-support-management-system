...
function Header()
    {
        ...
        $this->Cell(276,5,'Customer List',0,0,'C');
...
// Page footer
    ...
    function viewTable($db){
        ...
        $stmt = $db->query("SELECT *,concat(lastname,', ',firstname,' ',middlename) as name FROM customers order by concat(lastname,', ',firstname,' ',middlename) asc");
        while($data = $stmt->fetch(PDO::FETCH_OBJ)){
            ...
            $this->Cell(60,10,$data->contact,1,0,'L');
...
$pdf->Output();
?>
    // $result = mysqli_query($conn, "SELECT id, firstname,lastname,middlename, contact, address, email, date_created FROM customers") or die("database error:". mysqli_error($conn));